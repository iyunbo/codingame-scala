...
class BloxorzSuite {
  trait SolutionChecker extends GameDef with Solver with StringParserTerrain {
    ...
    def solve(ls: List[Move]): Block =
      ls.foldLeft(startBlock) { case (block, move) =>
        println(s"$block: $move")
        require(block.isLegal, s"block: $block is illegal") // The solution must always lead to legal blocks
        move match {
  ...
  @Test def `get neighbers with history`: Unit =
    new Level1 {
      assertEquals(neighborsWithHistory(Block(Pos(1,1),Pos(1,1)), List(Left,Up)).to(Set), Set(
        (Block(Pos(1,2),Pos(1,3)), List(Right,Left,Up)),
        (Block(Pos(2,1),Pos(3,1)), List(Down,Left,Up))
      ))
    }

  @Test def `get new neighbers only`: Unit =
    new Level1 {
      assertEquals(
        newNeighborsOnly(
          Set(
