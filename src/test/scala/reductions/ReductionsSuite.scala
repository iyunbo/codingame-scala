...
class ReductionsSuite {
  ...
  @Test def test_balance(): Unit = {
    assert(parBalance("()".toCharArray, 1))
  }

  @Test def test_balance2(): Unit = {
    assert(parBalance("(())".toCharArray, 1))
  }

  @Test def test_balance3(): Unit = {
    assert(!parBalance(")(".toCharArray, 1))
  }

  @Test def test_balance4(): Unit = {
    assert(parBalance("((()))".toCharArray, 1))
  }


  @Rule def individualTestTimeout = new org.junit.rules.Timeout(100000 * 1000)
}
