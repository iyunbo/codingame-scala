...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
                       explored: Set[Block]): LazyList[(Block, List[Move])] = neighbors filter (p => !explored.contains(p._1))
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) {
      ...
      val newNeighbors = newNeighborsOnly(neighborsWithHistory(current._1, current._2), explored)
      // BFS: we visit all neighbors before moving to next level
      current +: from(initial.tail ++ newNeighbors, explored + current._1)
    }
  }
  ...
  lazy val solution: List[Move] = {
    if (pathsToGoal.isEmpty) {
      ...
      val shortest = pathsToGoal.head
      println(s"solution: ${shortest}")
      shortest._2.reverse
    }
  }
}
