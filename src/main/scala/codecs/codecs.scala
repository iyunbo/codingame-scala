 ...
 *
 *  -A indicates that Encoder[Animal] would be the supertype of of Encoder[LivingBeing]
  */
trait Encoder[-A] {
...
trait EncoderInstances {
  ...
  implicit val intEncoder: Encoder[Int] =
    Encoder.fromFunction(n => Json.Num(BigDecimal(n))) // the json number type can have infinite number of decimal
...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] = Decoder.fromFunction {
    case i: Json.Num => if (i.value.isValidInt)
      Some(i.value.intValue)
    else
      None
    case _ => None
  }
  ...
  implicit val stringDecoder: Decoder[String] = Decoder.fromFunction {
    case s: Json.Str => Some(s.value)
    case _ => None
  }

  /** A decoder for `Boolean` values */
  implicit val booleanDecoder: Decoder[Boolean] = Decoder.fromFunction {
    case s: Json.Bool => Some(s.value)
    case _ => None
  }
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case lst: Json.Arr => Some((lst.items map decoder.decode).filter(_.isDefined).map(_.get))
      case _ => None
    }
